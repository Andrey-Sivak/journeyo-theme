...
>
    <div class="<?php echo $base_class . '__wrap'; ?>">
        ...
        <?php if (!empty($title)) : ?>
            <h2 class="<?php echo $base_class . '__title jn-animate'; ?>" id="reviews-section-title">
                <?php echo wp_kses_post($title); ?>
            </h2>
        <?php endif; ?>

        <?php
        $total_rating = array_sum(array_column($items, 'rating'));
                    ...
                    <?php foreach ($items as $index => $item) : ?>
                        ...
                        <li class="<?php echo $articleClass; ?>" itemscope itemtype="https://schema.org/Review">
                            <div class="<?php echo $base_class . '__item-wrap'; ?>">
                                ...
                                <div class="<?php echo $base_class . '__item-quote'; ?>" aria-hidden="true"
                                ...
                                <div class="<?php echo $base_class . '__item-content'; ?>">
                                    <?php if (!empty($item['name'])) : ?>
                                        <h3 class="<?php echo $base_class . '__item-name'; ?>" itemprop="author"
                                            itemscope itemtype="https://schema.org/Person">
                                            <span itemprop="name"><?php echo wp_kses_post($item['name']); ?></span>
                                        </h3>
                                    <?php endif; ?>
                                    <?php if (!empty($item['location'])) : ?>
                                        <p class="<?php echo $base_class . '__item-location'; ?>" itemprop="reviewBody">
                                    ...
                                    <?php if (isset($item['rating']) && $item['rating'] > 0 && $item['rating'] <= 5) : ?>
                                        ...
                                        >
                                            <meta itemprop="ratingValue"
                                                  content="<?php echo esc_attr($item['rating']); ?>">
                                            <meta itemprop="bestRating" content="5">
                                            <?php for ($i = 1; $i <= 5; $i++) : ?>
                                                <svg viewBox="0 0 26 25"
                                                     fill="<?php echo $i <= $item['rating'] ? '#FFCE08' : 'none'; ?>"
                                                     aria-label="<?php echo $i <= $item['rating'] ? 'Filled star' : 'Empty star'; ?>"
